...
useEffect(()=>{
  ...
  useEffect(()=>{
      if(location.pathname==="/services" || location.pathname==="/services/detailing" || location.pathname==="/services/paint-protection" || location.pathname === "/services/denting-and-painting" || location.pathname=== "/services/services" || location.pathname==="/services/accessories" ){
            setPath(true)
      }else{
      setPath(false)
      }
 ...
 const onOverlay = ()=>{
  ...
  return (
    ...
    <div className={isAboveMessageShow?'nav-shadow m-0 p-0 ':null}>
      ...
      {isAboveMessageShow? <div className='row position-relative  w-100 m-0 p-0  ' style={{position:'', background: 'linear-gradient(90deg, rgba(131,58,180,1) 0%, rgba(253,29,29,1) 47%, rgba(252,176,69,1) 100%)', height:'30px'}}>
       <div onClick={()=>navigate("/pre-delivery-inspection")} className='col-12 p-0 m-0 d-flex  align-items-center' style={{cursor:'pointer'}}>
       {/* <p className='pt-2 ps-2' style={{fontSize:'10px', color:'white', fontWeight:'600',textAlign:'justify'}}>Buying a new car ? Don’t risk it.
        Get a free pre delivery inspection now  &nbsp;
          <Link to="/pre-delivery-inspection" style={{fontSize:'10px', color:'#b8242a'}}>Explore...</Link>
           ...
           <marquee className='text-center ' style={{fontSize:'0.7rem', color:'white', fontWeight:'400', }} behavior="scroll" direction="left" scrollamount="10">
                        Safety First, Always: Our Pre-Delivery Inspection Ensures It. Dive into the Details
      </marquee>
       </div>
       <div className=" d-flex justify-content-center align-items-center position-absolute" style={{width:'30px', height:'30px', right:'8px', zIndex:'99'}}>
       <IconButton  onClick={()=>setIsAboveMessageShow(false)} aria-label="delete" size="small">
    ...
    <nav className={isAboveMessageShow?"navbar bg-body-tertiary":"navbar bg-body-tertiary fixed-top"} style={{backgroundColor:'white'}}>
      ...
      <img src={brandLogo?brandLogo:brandlogo} alt="Bootstrap" style={{maxWidth:'180px'}} className='img-fluid' />
        ...
        <div className="offcanvas-body" style={{backgroundColor:'white', zIndex:'99999'}}>
          <ul className="navbar-nav justify-content-end flex-grow-1 pe-3">
            ...
            <li className="nav-item" data-bs-toggle="offcanvas" data-bs-target="#offcanvasNavbar" aria-controls="offcanvasNavbar">
            <NavLink  className="nav-link nav-text" to="/services/detailing">Services</NavLink>
            </li>
